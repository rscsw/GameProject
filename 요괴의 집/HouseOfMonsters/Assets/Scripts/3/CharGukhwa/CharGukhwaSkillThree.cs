...
public class CharGukhwaSkillThree : SkillClass
{
    public GameObject skillPanel;
    public Text skillNameText, skillInfoText;

    public CharGukhwa gukhwa;
    public CharGukhwaInitSkillThree init;
    //공격자
    ...
    void Update()
    {
        ...
        if(gukhwa.isMyTurn && init.isCooled && gukhwa.coolDownThree && init.cooltime > 0)
        {
            init.cooltime--;
            gukhwa.coolDownThree = false;
        }
        if(init.cooltime <= 0)
        {
            init.isCooled = false;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("WhiteRabbit"))
        {
            if(whiteRabbit.hp > 0)
            {
                whiteRabbit.hp -= (gukhwa.atk * 5) - whiteRabbit.def;
                gukhwa.hp -= gukhwa.hp / 2;
        ...
        if (other.CompareTag("PinkRabbit"))
        {
            if(pinkRabbit.hp > 0)
            {
                pinkRabbit.hp -= (gukhwa.atk * 5) - pinkRabbit.def;
                gukhwa.hp -= gukhwa.hp / 2;
                init.cooltime = init._cooltime + 1;
                init.isSelceted = false;
                gukhwa.isSkilled = true;
